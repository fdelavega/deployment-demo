...
const CB_URL = process.env.CB_URL || "http://localhost:9090";

const app = express();
app.use(express.static(path.join(projectRoot, "frontend")));

app.get("/api/health", (_req, res) => {
  res.json({ ok: true, step: "01-baseline", source: CB_URL });
});
...
app.get("/api/entities", async (req, res) => {
  try {
    ...
    const url = new URL("/ngsi-ld/v1/entities", CB_URL);
...
app.listen(PORT, () => {
  console.log(`step-01 backend running on http://localhost:${PORT}`);
});
